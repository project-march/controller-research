...
float WATCHDOG_TIMER = 3.0;
bool PRE_CALIBRATED = true;

/**
 *
 *  Read JSON file from target
 *  @param endpoint odrive enumerated endpoint
 ...
 *
 */
int updateTargetConfig(odrive_endpoint* endpoint, const Json::Value& odrive_json, const string& config_file)
{
  ifstream cfg;
  string line, json;
...
{
  uint8_t u8val = AXIS_STATE_MOTOR_CALIBRATION;

  bool calibration_result = writeOdriveData(endpoint, odrive_json, string("axis0.requested_state"), u8val);
  ros::Duration(5.0).sleep();

  writeOdriveData(endpoint, odrive_json, string("axis0.motor.config.pre_calibrated"), PRE_CALIBRATED);

  writeOdriveData(endpoint, odrive_json, string("axis0.config.watchdog_timeout"), WATCHDOG_TIMER);
...
int getObjectByName(Json::Value odrive_json, std::string name, odrive_object* odo)
{
  ...
  string token;
  Json::Value js;
  Json::Value js2 = std::move(odrive_json);

  while (std::string::npos != (name.find('.')))
  {
    pos = name.find('.');
...
int execOdriveGetTemp(odrive_endpoint* endpoint, Json::Value odrive_json, std::string object, float& temp)
{
  ...
  ret = getObjectByName(std::move(odrive_json), std::move(object), &odo);
  if (ret)
  {
