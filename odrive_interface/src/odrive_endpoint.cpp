 ...
 *
 */
void odrive_endpoint::appendShortToCommBuffer(commBuffer& buf, short value)
{
 ...
 *
 */
commBuffer odrive_endpoint::decodeODrivePacket(commBuffer& buf, short& seq_no)
{
  commBuffer payload;
 ...
 *
 */
commBuffer odrive_endpoint::createODrivePacket(short seq_no, int endpoint_id, short response_size, bool read,
...
int odrive_endpoint::getData(int id, T& value)
{
  ...
  int result = endpointRequest(id, rx, rx_size, tx, 1 /* ACK */, sizeof(value));
  if (result != ODRIVE_OK)
  {
    return result;
  }
...
int odrive_endpoint::execFunc(int endpoint_id)
{
  ...
  return endpointRequest(endpoint_id, rx, rx_length, tx, true, 0);
}
 ...
 *
 */
int odrive_endpoint::endpointRequest(int endpoint_id, commBuffer& received_payload, int& received_length,
                                     commBuffer payload, bool ack, int length, bool read, int address)
{
  ...
  if (ack)
  {
    result = libusb_bulk_transfer(odrive_handle_, ODRIVE_IN_EP, receive_bytes, ODRIVE_MAX_BYTES_TO_RECEIVE,
